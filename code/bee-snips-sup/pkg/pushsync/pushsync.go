...
const (
	...
	streamName              = "pushsync"
	StreamNamePushsyncSNIPS = "pushsync-snips"
)
...
type Interface interface {
	ResetSkiplist(ctx context.Context) (bool, error)
}
...
func (s *PushSync) AddStreamer(streamer p2p.StreamerDisconnecter) {
	s.streamer = streamer
}
...
func (s *PushSync) Protocol() p2p.ProtocolSpec {
	return p2p.ProtocolSpec{
		...
		StreamSpecs: []p2p.StreamSpec{
			{
				Name:    streamName,
				Handler: s.handlerpushsync,
			},
			{
				Name:    StreamNamePushsyncSNIPS,
				Handler: s.handlersnips,
			},
		},
	}
}

func (ps *PushSync) handlerpushsync(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	return ps.handler(ctx, p, stream, false)
}

func (ps *PushSync) handlersnips(ctx context.Context, p p2p.Peer, stream p2p.Stream) error {
	err := ps.handler(ctx, p, stream, true)
	//ps.logger.Error(err, "handlersnips", "peer", p.Address.String())
	return err
}

// handler handles chunk delivery from other node and forwards to its destination node.
// If the current node is the destination, it stores in the local store and sends a receipt.
func (ps *PushSync) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream, snips bool) (err error) {
	if !*ps.Enabled {
		_ = stream.Reset()
		return fmt.Errorf("Pushsync is disabled. Enable using /debug/pushsync/enable or set --pushsync-enable: true.")
	}
	now := time.Now()
	...
	if err != nil {
		ps.logger.Error(err, "handlersnips: Could not unmarshal stamp.")
	...
	// If we are coming from SNIPS - always replicate.
	if p.FullNode || snips {
		if closer, _ := p.Address.Closer(chunkAddress, ps.address); closer || snips {
			...
			storagemode := storage.ModePutSync
			if snips {
				storagemode = storage.ModePutRequest
			}
			_, err = ps.storer.Put(ctxd, storagemode, chunk)
			if err != nil {
				return fmt.Errorf("chunk store: %w", err)
			}

			// Racin: Ignore accounting as we are not charging for replication.
...
func (ps *PushSync) ResetSkiplist(ctx context.Context) (bool, error) {
	ps.skipList.Reset()
	return true, nil
}
	...
	for {
		...
		case result := <-resultChan:
			...
			if ps.warmedUp() && !errors.Is(result.err, errNotAttempted) {
				ps.skipList.Add(ch.Address(), result.peer, sanctionWait) // Racin dont add to skiplist
...
func (ps *PushSync) pushPeer(ctx context.Context, resultChan chan<- receiptResult, doneChan <-chan struct{}, peer swarm.Address, ch swarm.Chunk, origin bool) {
	...
	// Racin: Just ignore this incentive things.
...
// Reset the skip list.
func (l *peerSkipList) Reset() {
	l.Lock()
	defer l.Unlock()
	l.skip = make(map[string]map[string]time.Time)
	l.resetTime = time.Now()
}

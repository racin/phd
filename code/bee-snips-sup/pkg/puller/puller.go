...
type Interface interface {
	ClearCursors(ctx context.Context, base, peer swarm.Address) // Racin - clear cursors for a peer
}
...
func (p *Puller) manage(warmupTime time.Duration) {
	...
	for {
		select {
		case <-c:
			// Racin. If pullsync is disabled. Don't do anything here.
			if !*p.enabled {
				continue
			}
			...
			// sync with that one
			p.logger.Debug("puller: Got notifyPeer signal")
			...
			_ = syncRadius // Racin - For Pullsync and SNIPS we use neighborhoodDepth, but syncradius might be used in the future.
			...
			_ = p.topology.EachPeerRev(func(peerAddr swarm.Address, po uint8) (stop, jumpToNext bool, err error) {
				if po >= neighborhoodDepth {
					...
					if _, ok := bp[peerAddr.ByteString()]; !ok {
						...
						p.logger.Debug("puller: Want to sync with peer.", "addr", peerAddr)
					} else {
						// already syncing, recalc
						peersToRecalc = append(peersToRecalc, peer{addr: peerAddr, po: po})
						p.logger.Debug("puller: Want to recalc with peer.", "addr", peerAddr)
					}
				}
			...
			for _, v := range peersToSync {
				p.syncPeer(ctx, v.addr, v.po, neighborhoodDepth)
			}

			for _, v := range peersToRecalc {
				dontSync := p.recalcPeer(ctx, v.addr, v.po, neighborhoodDepth)
...
func (p *Puller) disconnectPeer(peer swarm.Address, po uint8) {
	...
	p.logger.Debug("puller disconnect cleanup peer", "peer_address", peer, "proximity_order", po)
...
func (p *Puller) recalcPeer(ctx context.Context, peer swarm.Address, po, d uint8) (dontSync bool) {
	...
	p.logger.Debug("puller recalculating peer", "peer_address", peer, "proximity_order", po, "depth", d)
...
func (p *Puller) ClearCursors(ctx context.Context, base, peer swarm.Address) {
	p.cursorsMtx.Lock()
	initInterval := intervalstore.NewIntervals(1)
	if peer.Equal(swarm.ZeroAddress) {
		for peerAddr := range p.cursors {
			for bin := uint8(0); bin < p.bins; bin++ {
				key := peerIntervalKey(swarm.NewAddress([]byte(peerAddr)), bin)
				p.statestore.Put(key, initInterval)
			}
		}
		p.cursors = make(map[string]peerCursors)
		p.cursorsMtx.Unlock()
		p.syncPeersMtx.Lock()
		p.syncPeers = make([]map[string]*syncPeer, p.bins)
		for i := uint8(0); i < p.bins; i++ {
			p.syncPeers[i] = make(map[string]*syncPeer)
		}
	} else {
		delete(p.cursors, peer.ByteString())
		for i := uint8(0); i < p.bins; i++ {
			key := peerIntervalKey(peer, i)
			p.statestore.Put(key, initInterval)
		}
		p.cursorsMtx.Unlock()
		po := swarm.Proximity(base.Bytes(), peer.Bytes())
		p.syncPeersMtx.Lock()
		delete(p.syncPeers[po], peer.ByteString())
	}
	p.syncPeersMtx.Unlock()
...
func (p *Puller) syncPeer(ctx context.Context, peer swarm.Address, po, d uint8) {
	...
	if !ok {
		...
		if err != nil {
			p.logger.Debug("could not get cursors from peer", "peer_address", peer, "error", err)
			p.syncPeersMtx.Lock()
			delete(p.syncPeers[po], peer.ByteString())
			p.syncPeersMtx.Unlock()
...
func (p *Puller) histSyncWorker(ctx context.Context, peer swarm.Address, bin uint8, cur uint64) {
	...
	for {
		...
		select {
		case <-p.quit:
			p.logger.Debug("histSyncWorker quitting on shutdown", "peer_address", peer, "bin", bin, "cursor", cur)
			return
		case <-ctx.Done():
			p.logger.Debug("histSyncWorker context cancelled", "peer_address", peer, "bin", bin, "cursor", cur)
			return
		default:
		}
		...
		if err != nil {
			p.logger.Debug("histSyncWorker syncing interval failed", "peer_address", peer, "bin", bin, "cursor", cur, "error", err)
			if ruid == 0 {
				p.metrics.HistWorkerErrCounter.Inc()
			}
			...
			if err := p.syncer.CancelRuid(ctxC, peer, ruid); err != nil {
				p.logger.Debug("histSyncWorker cancel ruid failed", "error", err)
			}
			return
		}
		...
		p.logger.Debug("histSyncWorker pulled", "bin", bin, "start", s, "topmost", top, "peer_address", peer)
	}
}
...
func (p *Puller) liveSyncWorker(ctx context.Context, peer swarm.Address, bin uint8, cur uint64) {
	...
	for {
		...
		select {
		case <-p.quit:
			p.logger.Debug("liveSyncWorker quit on shutdown", "peer_address", peer, "bin", bin, "cursor", cur)
			return
		case <-ctx.Done():
			p.logger.Debug("liveSyncWorker context cancelled", "peer_address", peer, "bin", bin, "cursor", cur)
			return
		default:
		}
		top, ruid, err := p.syncer.SyncInterval(ctx, peer, bin, from, pullsync.MaxCursor)
		if err != nil {
			p.logger.Debug("liveSyncWorker exit on sync error", "peer_address", peer, "bin", bin, "from", from, "error", err)
			if ruid == 0 {
		...
		p.logger.Debug("liveSyncWorker pulled bin failed", "bin", bin, "from", from, "topmost", top, "peer_address", peer)

...
func New(streamer p2p.Streamer, storage pullstorage.Storer, unwrap func(swarm.Chunk), validStamp postage.ValidStampFn, logger log.Logger, o Options) *Syncer {
	return &Syncer{
		streamer:   streamer,
		storage:    storage,
		metrics:    newMetrics(),
...
func (s *Syncer) SyncInterval(ctx context.Context, peer swarm.Address, bin uint8, from, to uint64) (topmost uint64, ruid uint32, err error) {
	...
	defer func() {
		if err != nil {
			_ = stream.Reset()
			s.logger.Debug("error syncing peer", "peer_address", peer, "ruid", ru.Ruid, "bin", bin, "from", from, "to", to, "error", err)
		} else {
			go stream.FullClose()
		}
	}()
	...
	s.logger.Debug("syncing peer", "peer_address", peer, "ruid", ru.Ruid, "bin", bin, "from", from, "to", to)
	...
	if len(offer.Hashes) == 0 {
		s.metrics.TotalZeroHashes.Inc() // Racin metrics
	...
	s.metrics.TotalWantSent.Inc()                             // Racin metrics
	s.metrics.TotalChunksInWant.Add(float64(len(wantChunks))) // Racin metrics
	s.metrics.SizeOfWantSent.Add(float64(len(bv.Bytes())))    // Racin metrics
	...
	for ; ctr > 0; ctr-- {
		...
		s.metrics.Delivered.Inc() // Counter for delivered chunks.
...
func (s *Syncer) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	if !*s.Enabled {
		_ = stream.Reset()
		return fmt.Errorf("Pullsync is disabled. Enable using /debug/pullsync/enable or set --pullsync-enable: true.")
	}
	loggerV2 := s.logger.V(2).Register()
	_ = loggerV2
	...
	s.metrics.TotalRuidRecv.Inc() // Racin metrics
	s.logger.Debug("peer pulling", "peer_address", p.Address, "ruid", ru.Ruid)
	...
	s.metrics.TotalRangeRecv.Inc() // Racin metrics
	...
	s.metrics.TotalOfferSent.Inc()                               // Racin metrics
	s.metrics.TotalChunksInOffer.Add(float64(len(offer.Hashes))) // Racin metrics
	...
	s.metrics.TotalWantRecv.Inc() // Racin metrics
	startWant := time.Now()
	chs, err := s.processWant(ctx, offer, &want)
	if err != nil {
		return fmt.Errorf("process want: %w", err)
	}
	s.metrics.ProcessWantTime.Observe(time.Since(startWant).Seconds()) // Racin metrics
	for _, v := range chs {
	...
	s.metrics.UploadChunksTime.Observe(time.Since(startWant).Seconds()) // Racin metrics
...
func (s *Syncer) makeOffer(ctx context.Context, rn pb.GetRange) (o *pb.Offer, addrs []swarm.Address, err error) {
	...
	s.metrics.MakeOfferTime.Observe(time.Since(start).Seconds()) // Racin metrics
	return o, chs, nil
}
...
func (s *Syncer) GetCursors(ctx context.Context, peer swarm.Address) (retr []uint64, err error) {
	...
	s.logger.Debug("pullsync: getting cursors from peer", "peer_address", peer)
	defer func() {
		if err != nil {
			_ = stream.Reset()
	...
	s.metrics.TotalCursorReqSent.Inc() // Racin metrics
	var ack pb.Ack
	if err = r.ReadMsgWithContext(ctx, &ack); err != nil {
		return nil, fmt.Errorf("read ack: %w", err)
	}
	s.metrics.TotalCursorRecv.Inc() // Racin metrics
	retr = ack.Cursors
...
func (s *Syncer) cursorHandler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	s.metrics.TotalCursorReqRecv.Inc() // Racin metrics
	var ack pb.Ack
	s.metrics.DbOps.Inc()
	...
	s.metrics.TotalCursorSent.Inc()                           // Racin metrics
	s.metrics.SizeOfCursorSent.Add(float64(len(ack.Cursors))) // Racin metrics
	return nil
}
...
func (s *Syncer) CancelRuid(ctx context.Context, peer swarm.Address, ruid uint32) (err error) {
	...
	s.logger.Debug("sending ruid cancellation", "ruid", ruid, "peer_address", peer)
	w := protobuf.NewWriter(stream)
	defer func() {
		if err != nil {
			_ = stream.Reset()
			s.logger.Debug("error sending ruid cancellation failed", "ruid", ruid, "peer_address", peer, "error", err)
...
func (s *Syncer) cancelHandler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	defer func() {
		if err != nil {
			_ = stream.Reset()
			s.logger.Debug("cancellation failed", "peer_address", p.Address, "ruid", c.Ruid, "error", err)
		} else {
			_ = stream.FullClose()
		}
	}()
	...
	s.logger.Debug("cancelling", "peer_address", p.Address, "ruid", c.Ruid)

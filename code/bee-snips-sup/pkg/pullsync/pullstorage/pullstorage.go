...
func New(storer storage.Storer, o Options) *PullStorer {
	return &PullStorer{
		Storer:  storer,
		Options: o,
		metrics: newMetrics(),
...
func (s *PullStorer) IntervalChunks(ctx context.Context, bin uint8, from, to uint64, limit int) ([]swarm.Address, uint64, error) {
	...
	LOOP:
		for limit > 0 {
			...
			// 	count++
			// 	fmt.Printf("|RACIN| : Waited 30 seconds and nothing happened. Limit: %v. Timer: %v, ID: %d, Count: %d, Bin: %d. Key: %v\n", limit, timer, rndvalue, count, bin, fmt.Sprintf("%v-%v-%v-%v", bin, from, to, limit))
			case v, ok := <-ch:
				if !ok {
					nomore = true
					break LOOP

...
func (s *Service) mountTechnicalDebug() {
	...
	// Database debug API
	s.router.Handle("/debug/dbindices", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.debugDBIndicesHandler),
	})

	s.router.Handle("/debug/dbcontents", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.debugDBContentsHandler),
	})

	s.router.Handle("/debug/dblastchunks", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.debugDBLastIndexHandler),
	})

	// Syncing debug API
	s.router.Handle("/debug/pushsync", jsonhttp.MethodHandler{
		"GET":    http.HandlerFunc(s.pushsyncGetStateHandler),
		"DELETE": http.HandlerFunc(s.pushsyncClearSkiplistHandler),
	})
	s.router.Handle("/debug/pushsync/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.pushsyncUpdateStateHandler),
	})

	s.router.Handle("/debug/pullsync", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.pullsyncGetStateHandler),
	})
	s.router.Handle("/debug/pullsync/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.pullsyncUpdateStateHandler),
	})

	s.router.Handle("/debug/puller/cursors", jsonhttp.MethodHandler{
		"DELETE": http.HandlerFunc(s.pullerClearCursorHandler),
	})
	s.router.Handle("/debug/puller/cursors/{peeraddr}", jsonhttp.MethodHandler{
		"DELETE": http.HandlerFunc(s.pullerClearCursorHandler),
	})

	s.router.Handle("/debug/retrievalsync", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.retrievalsyncGetStateHandler),
	})
	s.router.Handle("/debug/retrievalsync/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.retrievalsyncUpdateStateHandler),
	})

	s.router.Handle("/debug/chainsync", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.chainsyncGetStateHandler),
	})
	s.router.Handle("/debug/chainsync/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.chainsyncUpdateStateHandler),
	})

	s.router.Handle("/debug/pos", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.posGetStateHandler),
	})
	s.router.Handle("/debug/pos/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.posUpdateStateHandler),
	})

	s.router.Handle("/debug/snips", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.snipsGetStateHandler),
	})
	s.router.Handle("/debug/snips/{newstate}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.snipsUpdateStateHandler),
	})

	s.router.Handle("/debug/notifyPeerSig", jsonhttp.MethodHandler{
		"POST": http.HandlerFunc(s.notifyPeerSigHandler),
	})

	s.router.Handle("/debug/broadcastKnownPeers", jsonhttp.MethodHandler{
		"POST": http.HandlerFunc(s.broadcastKnownPeers),
	})

	s.router.Handle("/debug/traverse/{root}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.traverseHandler),
	})

	// Snapshot debug API
	s.router.Handle("/debug/snapshot", jsonhttp.MethodHandler{
		"GET":    http.HandlerFunc(s.snapshotGetHandler),
		"DELETE": http.HandlerFunc(s.snapshotClearHandler),
	})

	s.router.Handle("/debug/snapshot/checksum", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.snapshotGetChecksumHandler),
	})

	s.router.Handle("/debug/snapshot/add", jsonhttp.MethodHandler{
		"POST": http.HandlerFunc(s.snapshotAddHandler),
	})

	s.router.Handle("/debug/snapshot/apply", jsonhttp.MethodHandler{
		"POST": http.HandlerFunc(s.snapshotApplyHandler),
	})
...
func (s *Service) mountAPI() {
	...
	handle("/pos/{reference}", web.ChainHandlers(
		web.FinalHandler(jsonhttp.MethodHandler{
			"GET": http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_, _ = w.Write([]byte("Access GET"))
				http.NotFound(w, r)
			}),
			"POST": http.HandlerFunc(s.posHandler),
		})),
	)

	handle("/snips", web.ChainHandlers(
		web.FinalHandler(jsonhttp.MethodHandler{
			"OPTION": http.HandlerFunc(s.snipsOptionHandler),
		}),
	))

	handle("/snips/{nonce}", web.ChainHandlers(
		web.FinalHandler(jsonhttp.MethodHandler{
			"GET": http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_, _ = w.Write([]byte("Nonce" + mux.Vars(r)["nonce"]))
				http.NotFound(w, r)
			}),
			"POST": http.HandlerFunc(s.snipsHandler),
		})),
	)
...
func (s *Service) mountBusinessDebug(restricted bool) {
	...
	handle("/neighbors", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.neighborsHandler),
	})

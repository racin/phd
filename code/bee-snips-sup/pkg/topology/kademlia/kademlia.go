...
const (
	maxConnAttempts        = 100 // when there is maxConnAttempts failed connect calls for a given peer it is considered non-connectable
	maxBootNodeAttempts    = 50  // how many attempts to dial to boot-nodes before giving up
	...
	flagTimeout      = 999999 * time.Hour // how long before blocking a flagged peer
	blockDuration    = time.Second * 10   // how long to blocklist an unresponsive peer for
	blockWorkerWakup = time.Hour * 10     // wake up interval for the blocker worker
)

var (
	nnLowWatermark              = 3 // the number of peers in consecutive deepest bins that constitute as nearest neighbours
	saturationPeers             = 8
	overSaturationPeers         = 100
	bootNodeOverSaturationPeers = 100
	shortRetry                  = 30 * time.Second
	timeToRetry                 = 2 * shortRetry
	broadcastBinSize            = 4
	peerPingPollTime            = 120 * time.Second // how often to ping a peer
)
...
func New(
	...
	if o.StartWithEmptyTopology {
		err = imc.Finalize(time.Now(), true)
		if err != nil {
			return nil, fmt.Errorf("failed to clear counters: %w", err)
		}
	}
	...
	k = &Kad{
		...
		startEmpty:         o.StartWithEmptyTopology,
		startTime:          start,
		bootnodePruneAfter: o.BootnodePruneAfter,
	...
	for {
		...
		case <-k.manageC:
			...
			if k.bootnode {
				...
				// Prune connections after BBchain peers are booted.
				if time.Now().After(k.startTime.Add(k.bootnodePruneAfter)) {
					k.logger.Debug("kademlia: bootnode pruning connection after boot.", "limit", k.bootnodePruneAfter, "time since boot", time.Since(k.startTime))
					k.pruneFunc(depth)
				}
...
func (k *Kad) connect(ctx context.Context, peer swarm.Address, ma ma.Multiaddr) error {
	...
	case err != nil:
		...
		//ss := k.collector.Inspect(peer)
		quickPrune := false //(ss == nil || ss.HasAtMaxOneConnectionAttempt()) && isNetworkError(err)
...
func (k *Kad) broadCastPeersWithDeadline(addressee swarm.Address, peers ...swarm.Address) {
	cCtx, cCancel := context.WithTimeout(k.bgBroadcastCtx, time.Minute)
	defer cCancel()

	if err := k.discovery.BroadcastPeers(cCtx, addressee, peers...); err != nil {
		k.logger.Debug("kademlia: broadCastPeersWithDeadline: Could not send known peers to %s. Err: %v", addressee, err)
	}
}

func (k *Kad) BroadcastKnownPeers(ctx context.Context) error {
	if !k.bootnode {
		return fmt.Errorf("BroadcastKnownPeers should only be called by bootnode.")
	}
	// batch in k.hive.limitBurst
	batchSize := 4 * int(swarm.MaxBins)
	peersBatch := make([]swarm.Address, 0)
	allKnownPeers := make([]swarm.Address, 0)
	allKnownPeersBatches := make([][]swarm.Address, 0)
	k.knownPeers.EachBinRev(func(peer swarm.Address, po uint8) (stop bool, jumpToNext bool, err error) {
		k.logger.Debug("kademlia: BroadcastKnownPeers: adding peer %v. PO %v. Batch %v", peer, po, len(allKnownPeersBatches))
		allKnownPeers = append(allKnownPeers, peer)
		peersBatch = append(peersBatch, peer)
		if len(peersBatch) == batchSize {
			allKnownPeersBatches = append(allKnownPeersBatches, peersBatch)
			peersBatch = make([]swarm.Address, 0)
		}
		return false, false, nil
	})
	...
	for _, peerToSendTo := range allKnownPeers {
		for i, batch := range allKnownPeersBatches {
			k.logger.Debug("kademlia: BroadcastKnownPeers: sending batch %v to peer %v.", i, peerToSendTo)
			k.broadCastPeersWithDeadline(peerToSendTo, batch...)
		}
	}
...
func (k *Kad) Pick(peer p2p.Peer) bool {
	...
	//if time.Now().Before(k.startTime.Add(k.bootnodePruneAfter)) {
	return true // Racin: Accept all connections during boot.
	//}
...
func (k *Kad) Connected(ctx context.Context, peer p2p.Peer, forceConnection bool) (err error) {
	...
	// Racin fix: Do not disconnect other peers if bootnode.
	// Racin: Accept all connections during boot.
	if time.Now().After(k.startTime.Add(k.bootnodePruneAfter)) {
...
func (k *Kad) Disconnected(peer p2p.Peer) {
	// if k.bootnode {
	// 	return // bootnode does not disconnect.
	// }
	k.logger.Info("kademlia: disconnected peer %s", peer.Address)
...
func (k *Kad) notifyPeerSig() {
	...
	k.logger.Debug("kademlia: notifyPeerSig called.")

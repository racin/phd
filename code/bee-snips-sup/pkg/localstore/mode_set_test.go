...
func TestModeSetRemoveAndThenPut(t *testing.T) {
	...
	for _, tc := range multiChunkTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Run("retrieve indexes", func(t *testing.T) {
				t.Run("retrieve data index count", newItemsCountTest(db.retrievalDataIndex, tc.count))
				t.Run("retrieve access index count", newItemsCountTest(db.retrievalAccessIndex, tc.count))
			})

			for _, ch := range chunks {
				newPushIndexTest(db, ch, wantTimestamp, nil)
			}

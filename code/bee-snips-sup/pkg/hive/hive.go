...
func (s *Service) BroadcastPeers(ctx context.Context, addressee swarm.Address, peers ...swarm.Address) error {
	...
	for len(peers) > 0 {
		...
		//s.logger.Debugf("hive: BroadcastPeers: sending %v peers to %v", max, addressee)
...
func (s *Service) sendPeers(ctx context.Context, peer swarm.Address, peers []swarm.Address) (err error) {
	...
	//s.logger.Debugf("hive: sendPeers: with %v peers. TO: %v", len(peers), peer)
	...
	for _, p := range peers {
		...
		if !s.allowPrivateCIDRs && isPeerPublic && manet.IsPrivateAddr(addr.Underlay) {
			s.logger.Debug("hive: sendPeers: Don't advertise private CIDRs to the public network.")
...
func (s *Service) peersHandler(ctx context.Context, peer p2p.Peer, stream p2p.Stream) error {
	...
	//s.logger.Debugf("hive: peersHandler: Handling %v peers.", len(peersReq.Peers))
	if !s.inLimiter.Allow(peer.Address.ByteString(), len(peersReq.Peers)) {
		_ = stream.Reset()
		return ErrRateLimitExceeded
	}
	//s.logger.Debugf("hive: peersHandler: inLimiter did not exceeded. %v", len(peersReq.Peers))
	...
	go func() {
		...
		for {
			...
			case newPeers := <-s.peersChan:
				//s.logger.Debugf("hive: startCheckPeersHandler: Adding %v peers.", len(newPeers.Peers))
				s.wg.Add(1)
				go func() {
					defer s.wg.Done()

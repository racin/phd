...
type Options struct {
	...
	DepthMonitorEnabled           bool
	///////////////// Racin
	ChainEnable             bool
	IgnoreRadius            bool
	PushSyncEnabled         bool
	PullSyncEnabled         bool
	RetrievalSyncEnabled    bool // Syncing that occurs when a chunk is forwarded from A to B via C. (A->C->B)
	StartWithEmptyTopology  bool // clear address book and don't load topology snapshot on start
	ChainSyncEnabled        bool // Spams other peers to check whether they are up to date with the blockchain. Not needed in bbchain.
	POSEnabled              bool
	BootnodePruneAfter      time.Duration
	HostName                string
	SNIPSEnabled            bool
	SNIPSminGamma           float64
	SNIPSmaxGamma           float64
	SNIPSblockInterval      uint64
	SNIPSblockCheckInterval time.Duration
}
...
func NewBee(interrupt chan struct{}, sysInterrupt chan os.Signal, addr string, publicKey *ecdsa.PublicKey, signer crypto.Signer, networkID uint64, logger log.Logger, libp2pPrivateKey, pssPrivateKey *ecdsa.PrivateKey, o *Options) (b *Bee, err error) {
	...
	if o.StartWithEmptyTopology {
		overlays, err := addressbook.Overlays()
		if err != nil {
			return nil, fmt.Errorf("failed to get overlay addresses for removal: %w", err)
		}
		for _, addr := range overlays {
			err = addressbook.Remove(addr)
			if err != nil {
				return nil, fmt.Errorf("failed to remove overlay address: %w", err)
			}
		}
	}
	...
	kad, err := kademlia.New(swarmAddress, addressbook, hive, p2ps, pingPong, metricsDB, logger,
		kademlia.Options{Bootnodes: bootnodes, BootnodeMode: o.BootnodeMode, StaticNodes: o.StaticNodes, IgnoreRadius: !o.ChainEnable || o.IgnoreRadius, BootnodePruneAfter: o.BootnodePruneAfter})
	if err != nil {
	...
	retrieve := retrieval.New(swarmAddress, storer, p2ps, kad, logger, acc, pricer, tracer, o.RetrievalCaching, validStamp, retrieval.Options{
		Enabled: &o.RetrievalSyncEnabled,
	})
	...
	pushSyncProtocol := pushsync.New(swarmAddress, nonce, p2ps, storer, kad, tagService, o.FullNodeMode, pssService.TryUnwrap, validStamp, logger, acc, pricer, signer, tracer, warmupTime, pushsync.Options{
		Enabled: &o.PushSyncEnabled,
	})
	...
	pullStorage := pullstorage.New(storer, pullstorage.Options{
		Enabled: &o.PullSyncEnabled,
	})

	pullSyncProtocol := pullsync.New(p2ps, pullStorage, pssService.TryUnwrap, validStamp, logger, pullsync.Options{
		Enabled: &o.PullSyncEnabled,
	})
	b.pullSyncCloser = pullSyncProtocol

	var pullerService *puller.Puller
	if o.FullNodeMode { //&& !o.BootnodeMode { (Racin: Bootnode will act as a full node)
		pullerService = puller.New(stateStore, kad, batchStore, pullSyncProtocol, logger, puller.Options{
			Enabled: &o.PullSyncEnabled,
		}, warmupTime)
		b.pullerCloser = pullerService
	}

	posService := pos.New(swarmAddress, nonce, networkID, logger, pushSyncProtocol, signer, storer, p2ps, kad, pos.Options{
		Enabled: &o.POSEnabled,
	})
	snipsOptions := snips.Options{
		Enabled:            &o.SNIPSEnabled,
		MinGamma:           &o.SNIPSminGamma,
		MaxGamma:           &o.SNIPSmaxGamma,
		BlockInterval:      &o.SNIPSblockInterval,
		BlockCheckInterval: &o.SNIPSblockCheckInterval,
	}
	snipsService := snips.New(swarmAddress, nonce, networkID, logger, batchStore, signer, storer, p2ps, kad, chainBackend, snipsOptions)
	...
	posProtocolSpec := posService.Protocol()
	snipsProtocolSpec := snipsService.Protocol()
	...
	if o.FullNodeMode {
		...
		p2p.WithBlocklistStreams(p2p.DefaultBlocklistTime, posProtocolSpec)
		p2p.WithBlocklistStreams(p2p.DefaultBlocklistTime, snipsProtocolSpec)
	}
	...
	if err = p2ps.AddProtocol(posProtocolSpec); err != nil {
		return nil, fmt.Errorf("pos protocol: %w", err)
	}
	if err = p2ps.AddProtocol(snipsProtocolSpec); err != nil {
		return nil, fmt.Errorf("snips protocol: %w", err)
	}
	...
	if o.FullNodeMode && o.DepthMonitorEnabled {
	...
	if o.FullNodeMode {
		...
		chainSyncer, err = chainsyncer.New(chainBackend, cs, kad, p2ps, logger, &chainsyncer.Options{
			FlagTimeout:     20 * time.Minute,
			PollEvery:       10 * time.Minute,
			BlockerPollTime: 10 * time.Second,
			Enabled:         &o.ChainSyncEnabled,
		})
		if err != nil {
	...
	extraOpts := api.ExtraOptions{
		...
		LocalStore:           storer,
		PushSyncEnabled:      &o.PushSyncEnabled,
		PullSyncEnabled:      &o.PullSyncEnabled,
		RetrievalSyncEnabled: &o.RetrievalSyncEnabled,
		PushSync:             pushSyncProtocol,
		ChainSyncEnabled:     &o.ChainSyncEnabled,
		Snips:                snipsService,
		SnipsOptions:         &snipsOptions,
		Puller:               pullerService,
		HostName:             o.HostName,
	}
	...
	if o.DebugAPIAddr != "" {
		...
		debugService.MustRegisterMetrics(posService.Metrics()...)
		debugService.MustRegisterMetrics(snipsService.Metrics()...)

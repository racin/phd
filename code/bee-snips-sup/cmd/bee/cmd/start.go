	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			b, err := node.NewBee(interruptChannel, sysInterruptChannel, c.config.GetString(optionNameP2PAddr), signerConfig.publicKey, signerConfig.signer, networkID, logger, signerConfig.libp2pPrivateKey, signerConfig.pssPrivateKey, &node.Options{
				...
				DepthMonitorEnabled:        c.config.GetBool(optionNameDepthMonitorEnabled),
				///// Racin
				PushSyncEnabled:         c.config.GetBool(optionNamePushSyncEnabled),
				PullSyncEnabled:         c.config.GetBool(optionNamePullSyncEnabled),
				RetrievalSyncEnabled:    c.config.GetBool(optionNameRetrievalCaching),
				StartWithEmptyTopology:  c.config.GetBool(optionNameStartWithEmptyTopology),
				ChainSyncEnabled:        c.config.GetBool(optionNameChainSyncEnabled),
				POSEnabled:              c.config.GetBool(optionNamePOSEnabled),
				BootnodePruneAfter:      c.config.GetDuration(optionBootnodePruneAfter),
				ChainEnable:             c.config.GetBool(optionNameChainEnable),
				IgnoreRadius:            c.config.GetBool(optionNameIgnoreRadius),
				HostName:                c.config.GetString(optionNameHostName),
				SNIPSEnabled:            c.config.GetBool(optionNameSNIPSEnabled),
				SNIPSminGamma:           c.config.GetFloat64(optionNameSNIPSmingamma),
				SNIPSmaxGamma:           c.config.GetFloat64(optionNameSNIPSmaxgamma),
				SNIPSblockInterval:      c.config.GetUint64(optionNameSNIPSblockinterval),
				SNIPSblockCheckInterval: c.config.GetDuration(optionNameSNIPSblockcheckinterval),
			})
			if err != nil {
				return err
			}

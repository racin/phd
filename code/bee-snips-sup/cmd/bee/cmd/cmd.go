...
const (
	...
	optionNameIgnoreRadius               = "ignore-radius"
	...
	optionNamePushSyncEnabled            = "pushsync-enable"
	optionNamePullSyncEnabled            = "pullsync-enable"
	optionNameStartWithEmptyTopology     = "start-with-empty-topology"
	optionNameChainSyncEnabled           = "chainsync-enable"
	optionNamePOSEnabled                 = "pos-enable"
	optionBootnodePruneAfter             = "bootnode-prune-after"
	optionNameStorageIncentivesEnable    = "storage-incentives-enable"
	optionNameDepthMonitorEnabled        = "depthmonitor-enable"
	optionNameHostName                   = "hostname"

	// SNIPS
	optionNameSNIPSEnabled            = "snips-enable"
	optionNameSNIPSmingamma           = "snips-mingamma"
	optionNameSNIPSmaxgamma           = "snips-maxgamma"
	optionNameSNIPSblockinterval      = "snips-blockinterval"
	optionNameSNIPSblockcheckinterval = "snips-blockcheckinterval"
)
...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Bool(optionNameIgnoreRadius, false, "ignore neighborhood radius")
	...
	cmd.Flags().Bool(optionNamePullSyncEnabled, true, "enable pullsync")
	cmd.Flags().Bool(optionNamePushSyncEnabled, true, "enable pushsync")
	cmd.Flags().Bool(optionNameStartWithEmptyTopology, false, "start with empty topology")
	cmd.Flags().Bool(optionNameChainSyncEnabled, false, "check if other peers are synced with blockchain")
	cmd.Flags().Bool(optionNamePOSEnabled, false, "enable proof-of-storage")
	cmd.Flags().String(optionNameHostName, "undefined", "hostname of the peer")
	cmd.Flags().Duration(optionBootnodePruneAfter, time.Hour*999999, "how much time until the bootnode starts pruning connections")
	cmd.Flags().Bool(optionNameStorageIncentivesEnable, false, "enable storage incentives feature")

	// SNIPS
	cmd.Flags().Bool(optionNameSNIPSEnabled, false, "enable SNIPS")
	cmd.Flags().Float64(optionNameSNIPSmingamma, 2, "minimum gammaa value for SNIPS")
	cmd.Flags().Float64(optionNameSNIPSmaxgamma, 3, "maximum gammaa value for SNIPS")
	cmd.Flags().Uint64(optionNameSNIPSblockinterval, 7200, "how many blocks to wait before sending a new SNIPS proof")
	cmd.Flags().Duration(optionNameSNIPSblockcheckinterval, 60*time.Second, "how frequent to check for new blocks")
}
